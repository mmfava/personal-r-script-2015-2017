...
# Exemplo 1: 
# Existe diferença em relação aos gêneros da turma de Ciências Biológicas?
sexo=c("feminino"=??, "masculino"=??)

# Considerações para a análise
# - 1º É esperado que as proporções sejam iguais.
# - 2º São X mulheres e Y homens --> n = ? (n > ou < 25?)
# - 3º A tabela possui k = ? e a FE = ? (FE > 5?) 
## !! Aplicar correção para continuidade de Yates !!

# ~ Teste chi-quadrado:
chisq.test(sexo, correct=TRUE) # chi-quadrado = ??, df = ??, P = ??
...
# Resposta:
# Existe associação entre as variáveis em estudo?
...
## Se você quiser buscar por outros pacotes que realizam as análises vistas aqui
## é só acessar pelo pacote "sos".
library(sos)
???"chi squared"
